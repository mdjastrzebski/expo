...
> **warning** The `fingerprint` runtime policy is still in beta as of SDK 51 and not yet considered stable. Use may result in unexpected system behavior.
...
This policy works for both projects with and without custom native code. It works by using the `@expo/fingerprint` package to calculate a hash of your project during builds and updates to determine build-update compatibility (a.k.a. the runtime).

### `"fingerprintExperimental"` runtime version policy

> **warning** `fingerprintExperimental`is deprecated and will be removed in SDK 51.
